...
export async function createPost(category: string, title: string, content: string) {
  const res = await httpPost<PostDetail>('/api/posts', { category, title, content })
  return res.data
}

export async function updatePost(id: string, title: string, content: string) {
  const res = await httpPut<PostDetail>(`/api/posts/${id}`, { title, content })
  return res.data
}
...
export async function createComment(postId: string, content: string) {
  const res = await httpPost<PostComment>(`/api/posts/${postId}/comments`, { content })
  return res.data
}
...
export async function getLikeStatus(postId: string) {
  const res = await httpGet<{ liked: boolean }>(`/api/posts/${postId}/likes/status`)
  return res.data
}
