...
const MOCK_TRAINS: Record<
  string,
  { id: string; name: string; depart: string; arrive: string; seats: number }[]
> = {
  daejeon: [
    {
      id: "ktx-101",
...
const SPOTS = [
  {
    ...
    category: "맛집 · 베이커리",
    description:
      "1956년부터 이어온 대전의 대표 빵집. 튀김소보로와 부추빵으로 유명한 전국구 명소.",
  ...
  },
  {
    ...
    category: "문화 · 과학",
    description:
      "과학과 기술의 역사를 한눈에. 아이들과 함께 오기 좋은 체험형 과학 전시관.",
...
function getSelectedTrainName() {
  return MOCK_TRAINS[selectedStation.value].find(
...
function getSelectedTrainDetails() {
  ...
  const trainList = MOCK_TRAINS[selectedStation.value] || [];
...
const selectedSpotForModal = ref<Spot | null>(null);

function openSpotModal(spot: Spot) {
                  ...
                  <div v-if="searchMode === 'train'">
                    ...
                    >
                      ...
                      <div style="max-height: 140px; overflow-y: auto">
                        <button
                          v-for="train in MOCK_TRAINS[selectedStation] || []"
                          :key="train.id"
                          type="button"
